...
drawer = pc.drawer(repetitions=pc.v.repetitions, humidity=pc.v.RH_levels, flowrate=pc.v.flow_rates)

# pc.v.flow_rate = 1030 # mL/min
pc.v.current_RH = pc.choice(drawer.original)['humidity']  # start at random RH
pc.v.current_flowrate = pc.choice(drawer.original)['flowrate']
pc.v.next_RH = pc.v.current_RH
pc.v.next_flowrate = pc.v.current_flowrate
...
def check_update_rewarded_side():
    sample = drawer.draw()
    pc.v.next_RH = sample['humidity']
    pc.v.next_flowrate = sample['flowrate']
...
def run_start():
    ...
    hygrostat.begin()
    hygrostat.set_flowrate(pc.v.next_flowrate)
    ...
    if event == "entry":
        ...
        if pc.v.n_total_trials > 0: # this will skip the initial ITI at run start
            ...
            pc.print_variables(["n_total_trials", "n_correct_trials", "n_early_errors",
                                ...
                                "choice", "outcome", "current_RH", "current_flowrate", "early_err_flag", "air_delivery_duration", "nested_var", "reward_durations", "n_rewards", "big_rwd_counter"])
        ...
        if not pc.v.early_err_flag: # only update if no early error on last trial
            ...
            pc.v.current_flowrate = pc.v.next_flowrate
