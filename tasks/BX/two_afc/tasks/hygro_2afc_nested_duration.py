...
pc.v.durations = [100, 200, 500, 1000]
pc.v.nested_var = "current_duration"
...
drawer = pc.drawer(repetitions=pc.v.repetitions, humidity=pc.v.RH_levels, duration=pc.v.durations)

pc.v.flow_rate = 1000 # mL/min
pc.v.current_RH = pc.choice(drawer.original)['humidity']  # start at random RH
pc.v.current_duration = pc.choice(drawer.original)['duration']
pc.v.next_RH = pc.v.current_RH
pc.v.next_duration = pc.v.current_duration
...
def check_update_rewarded_side():
    ...
    pc.v.next_duration = sample['duration']
...
def deliver_air(event):
    if event == "entry":
        ...
        pc.timed_goto_state("wait_for_side_poke", pc.v.current_duration)
    elif event == "exit":
...
def inter_trial_interval(event):
    if event == "entry":
        ...
        if not pc.v.early_err_flag: # only update if no early error on last trial
            ...
            pc.v.current_duration = pc.v.next_duration
